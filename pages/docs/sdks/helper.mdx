...
A collection of JS helper functions browsers and servers integrating Hellō.
...
- [provider hint](/docs/apis/wallet/#provider-hint) documentation
...
This is a helper function for the `https://wallet.hello.coop/oauth/introspection` API as described [here](/docs/oidc/verification#introspection). 
...
If successfully validated, you will receive the full [ID Token payload](/docs/oidc/verification#response) with `active: true` to indicate it is an active token. If unsuccessful, you will receive an [Introspection Errors](/docs/oidc/errors#introspection).
