...
Make sure you're using `https` in development or set `ENABLE_3P_COOKIES=true` for third-party cookie scenarios. The `secure: true` option requires HTTPS in production.

### *2) Cookies not working across subdomains?*

Set `HELLO_COOKIE_DOMAIN=.yourdomain.com` to enable cookies across all subdomains. Only use this if you control all subdomains for security.

### *3) Redirect URI discovery not working?*

Ensure your server can handle the discovery script. The SDK sends a JavaScript page that reads the current URL and redirects back with the discovered URI.

### *4) SameSite cookie issues?*

- Use `HELLO_SAME_SITE_STRICT=true` for maximum security (may cause additional redirects)
- Use `ENABLE_3P_COOKIES=true` for third-party scenarios (requires HTTPS)
- Default `sameSite: 'lax'` works for most use cases

### *5) Cookie encryption errors?*

Ensure `HELLO_COOKIE_SECRET` is set to a 32-character hex string. Generate one with:
```bash
