...
The cookies are encrypted using `aes-128-gcm` and the [`HELLO_COOKIE_SECRET`](./environment/#hello_cookie_secret)
...
- `sameSite: strict` - the browser will only send the cookie from pages that are in the same domain. This provides additional security, but adds complexity when responding to redirects. The SDKs employ a bounce to enable this restriction.
...
In contrast, NextAuth.js has set [`sameSite: lax`](https://github.com/nextauthjs/next-auth/blob/main/packages/core/src/lib/cookie.ts). OWASP has a [good explanation](https://owasp.org/www-community/SameSite) on the difference between setting `sameSite` to be `strict` vs `lax`.

### *5) Why are you not using `aes-256-gcm`?*

Encrypting the cookie is only one layer of defense. The cookie is already protected by TLS and is not available to the browser. 128 bit keys are considered adequate security, and 256 bit keys are typically only required for military applications. Why consume more resources than are needed?
...
### *2) What are Development Redirect URIs?*
...
### *3) What is Redirect URI Discovery?*
...
### *4) What is Redirect URI Auto Config?*
...
### *5) Why not just use `/api/hellocoop`?*
...
### *6) Why do the SDKs default to `/api/hellocoop`?*
