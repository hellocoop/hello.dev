...
If your did not use Hellō Quickstart, you will need to create an application at the [Hellō Developer Console](https://console.hello.coop/) and get a `client_id`.
...
`helloAuth()` will add the `/api/hellocoop` endpoint which will process the Hellō authorization response, and the [Web Client](/docs/api-web-client) `login`, `logout`, and `auth` operations.
