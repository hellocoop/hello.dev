...
Hello supports both the [Code Flow](https://openid.net/specs/openid-connect-core-1_0.html#CodeFlowAuth) and the [Implicit Flow](https://openid.net/specs/openid-connect-core-1_0.html#ImplicitFlowAuth)
...
|`response_type` (optional)| `id_token` (implicit flow) or<br/><span>`code` (code flow - default and recommended, but requires [PKCE - RFC7636](https://www.rfc-editor.org/rfc/rfc7636.html)).</span> |
...
|`code_challenge_method`|May be provided if `code_challenge` is included.<br/>MUST have value of `S256`. See PKCE below|
...
### Advanced Features
See the [API Reference | Wallet](/docs/apis/wallet/) for additional functionality including how to change the recommended providers with `provider_hint`.
