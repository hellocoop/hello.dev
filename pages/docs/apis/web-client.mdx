...
If there is an error response from Hello, the browser will be redirected to the error page with the `error` parameter. See [Error Response](/oidc/errors) for details on error response. See [`hello.config.js`](/docs/sdks/config/) for overriding the default error page.
