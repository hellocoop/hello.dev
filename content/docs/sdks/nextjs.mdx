...
>  Note they are available from `@hellocoop/nextjs`
...
```typescript
import { logOut, getLogOutRoute } from '@hellocoop/nextjs'
```

`logOut()` - function to logout user, loads `getLogOutRoute`

`getLogOutRoute` - provides route to logout
...
```ts
import { HelloProvider, LoggedIn, LoggedOut, ContinueButton, getServerSideProps } from '@hellocoop/nextjs'
...
// This is a convenience wrapper around `getAuth()`
...
export default async loginSyncCallback ({ 
    ...
    return { 
        ...
        updatedAuth: { email, name, picture, role }
    }
} 
```
  </Tabs.Tab>
