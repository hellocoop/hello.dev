Documentation[SDK References](/docs/sdks/)Express

# Express SDK

> **ℹ️ Info:** 

Follow the [Express Quickstart](/docs/quickstarts/express/) directions to add Hellō to your application in minutes.
...
If your did not use Hellō Quickstart, you will need to create an application at the [Hellō Developer Console (opens in a new tab)](https://console.hello.coop/) and get a `client_id`.

### 4\. `hello.config.js`[](#4-helloconfigjs)

The convention is to create a `hello.config.js` file in the root of your project for configuration. This file is imported into the `hellocoop.js` file and passed to the `pageAuth()` function for configuration. Below is a basic configuration file. See [`hello.config.js`](/docs/sdks/config/) for additional configuration.
...
Configures the SDK and adds the `/api/hellocoop` route. See [5\. Api Route](/docs/sdks/express/#5-api-route) for usage.
...
Would return a `401` for any unauthenticated call to `/api/*`.
...
[SDK References](/docs/sdks/ "SDK References")[Fastify](/docs/sdks/fastify/ "Fastify")